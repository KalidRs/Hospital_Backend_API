...
def get_horario(db: Session, horario_id: str):
...
def update_horario(db: Session, horario_id: str, horario_data: schemas.HorarioUpdate):
...
def delete_horario(db: Session, horario_id: str):
    db_horario = db.query(models.Horario).filter(models.Horario.id == horario_id).first()
    if db_horario:
