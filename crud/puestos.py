...
def get_puesto(db: Session, puesto_id: str):
...
def update_puesto(db: Session, puesto_id: str, puesto_data: schemas.PuestoUpdate):
...
def delete_puesto(db: Session, puesto_id: str):
    db_puesto = db.query(models.Puesto).filter(models.Puesto.id == puesto_id).first()
    if db_puesto:
        db.delete(db_puesto)
        db.commit()
